...
async def run_proposer_models(reference_models=reference_models):
...
async def main():
    ...
    responses = await run_proposer_models()
    modified_aggregator_system_prompt = aggreagator_system_prompt + "\n" + "\n".join([f"{i+1}. {str(element)}" for i, element in enumerate(responses)])
    console.print("[bold green]Aggregating response...[/bold green]")
    ...
    console.print(f"[bold red]Aggregator Response:[/bold red] {agg_response.choices[0].message.content}")
